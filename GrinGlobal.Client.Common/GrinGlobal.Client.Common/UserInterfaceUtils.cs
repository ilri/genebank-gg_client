                ...
                if (dataColumn.ExtendedProperties.Contains("gui_hint"))
                {
                    switch (dataColumn.ExtendedProperties["gui_hint"].ToString())
                    {
                        ...
                        default:
                            ...
                            {
                                newDataTable.Columns[newColumnName].DateTimeMode = DataSetDateTime.Unspecified;
                            }
                            break;
                    }
                }
                    ...
                    foreach (DataColumn dc in newDataTable.Columns)
                    {
                        ...
                        else
                        {
                            if (!string.IsNullOrEmpty(dr[dc.ColumnName].ToString()))
                            {
                                ...
                                {
                                    if ("created_date modified_date owned_date".Contains(dc.ColumnName))
                                    {
                                        // If the date field is created_date, modified_date, or owned_date it is stored in the database
                                        // as a UTC datetime by the middle tier.  When the datetime field is sent to the CT it has a DateTimeMode property of
                                        // "DataSetDateTime.Unspecified".  So convert the datetime to LocalTime using the ToLocalTime() method call...
                                        newDataRow[dc.ColumnName] = ((DateTime)dr[dc.ColumnName]).ToLocalTime();
//DateTime raw = (DateTime)dr[dc.ColumnName];
//DateTime local = ((DateTime)dr[dc.ColumnName]).ToLocalTime();
//DateTime utc = ((DateTime)dr[dc.ColumnName]).ToUniversalTime();
                                    }
                                }
                            }
                        }
            ...
            {
                // Turn on wrap mode for all columns set as varchar(max) and set the gridview column text...
                ...
                // Get the text for the column description...
                if (newDataTable.Columns[dgvc.Name].ExtendedProperties.Contains("description") &&
                    !string.IsNullOrEmpty(newDataTable.Columns[dgvc.Name].ExtendedProperties["description"].ToString()))
                {
                    dgvc.ToolTipText = newDataTable.Columns[dgvc.Name].ExtendedProperties["description"].ToString();
                }
            }
        }
        ...
        public void buildEditDataGridView(DataGridView dataGridView, DataTable dataTable, LookupTables lookupTables, string cno)
        {
            ...
            foreach (DataColumn dataColumn in dataTable.Columns)
            {
                ...
                if (dataColumn.ExtendedProperties.Contains("max_length") &&
                    dataColumn.ExtendedProperties["max_length"].ToString().Trim() == "-1")
                {
                    newDGVColumn.DefaultCellStyle.WrapMode = DataGridViewTriState.True;
                }
                ...
                // Get the text for the column description...
                if (dataColumn.ExtendedProperties.Contains("description") &&
                    !string.IsNullOrEmpty(dataColumn.ExtendedProperties["description"].ToString()))
                {
                    newDGVColumn.ToolTipText = dataColumn.ExtendedProperties["description"].ToString();
                }
...
if (dataColumn.ExtendedProperties.Contains("default_value") &&
    !string.IsNullOrEmpty(dataColumn.ExtendedProperties["default_value"].ToString()) &&
    dataColumn.ExtendedProperties["default_value"].ToString().Trim().ToUpper() != "{DBNULL.VALUE}")
{
    dataColumn.DefaultValue = dataColumn.ExtendedProperties["default_value"].ToString();
}
...
e.Handled = true;
        ...
        public bool ProcessDGVReadOnlyShortcutKeys(DataGridView dgv, KeyEventArgs e, string cno, LookupTables lookupTables)
        {
            bool keyProcessed = false;
            ...
            if (e.KeyCode == Keys.E && e.Control)
            {
                ...
                string currentCellValue = dgv.CurrentCell.Value.ToString();
                RichTextEditor rte = new RichTextEditor(currentCellValue, true);
                rte.ShowDialog();
            }
