...
splash.Update();
            ...
            {
                localDBInstance = _sharedUtils.Url.ToLower().Replace("http://", "").Replace("https://", "").Replace("/gringlobal/gui.asmx", "").Replace("/gringlobal_remote_debug/gui.asmx", "").Replace('-', '_').Replace('.', '_').Replace(':', '_');
...
//splash.Update();
            ...
            //ux_statusCenterMessage.TextAlign = statusCenterMessageAllignment;
            //ux_statusCenterMessage.Text = statusCenterMessage;
            //ux_statusRightProgressBar.Visible = statusRightProgressBarVisible;
            //ux_statusstripMain.Refresh();
...
//                        else
//                        {
//                            // The status of the lookup table is normal so compare the last sync date 
//                            // against the date the table was last touched to determine if an update is needed...
//                            if ((DateTime)dr["sync_date"] < (DateTime)dr["last_touched_date"])
//                            {
...
//string[] argsArray = new string[100];
//argsArray[0] = partiallyLoadedTables.ToString();
//ggMessageBox.MessageText = string.Format(ggMessageBox.MessageText, argsArray);
...
//string commandLineParameters = "-user=" + username + " -password=" + password + " -url=" + GRINGlobalWebServices.Url;
        ...
        private void GrinGlobalClient_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (!e.Control && ux_buttonEditData.Enabled)
            {
...
//_sharedUtils = new SharedUtils(GRINGlobalWebServices.Url, username, password, localDBInstance, cno);
                    ...
                    //localDBInstance = GRINGlobalWebServices.Url.ToLower().Replace("http://", "").Replace("https://", "").Replace("/gringlobal/gui.asmx", "").Replace("/gringlobal_remote_debug/gui.asmx", "").Replace('-', '_').Replace('.', '_').Replace(':', '_');
                    localDBInstance = _sharedUtils.Url.ToLower().Replace("http://", "").Replace("https://", "").Replace("/gringlobal/gui.asmx", "").Replace("/gringlobal_remote_debug/gui.asmx", "").Replace('-', '_').Replace('.', '_').Replace(':', '_');
                    localDBInstance = "GRINGlobal_" + localDBInstance;
...
//GRINGlobalWebServices.Url = originalURL;
        ...
        private void ux_buttonSaveUserSettingsNow_Click(object sender, EventArgs e)
        {
            ...
            // Save user treeview lists...
            SaveNavigatorTabControlData(_ux_NavigatorTabControl, int.Parse(_usernameCooperatorID));
            // Remember the current tab name
            string currentTabName = _ux_NavigatorTabControl.SelectedTab.Name;
            ...
            _ux_NavigatorTabControl.SelectTab(currentTabName);
