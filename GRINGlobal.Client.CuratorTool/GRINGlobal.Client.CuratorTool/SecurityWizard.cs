...
//}
            // Find all pkeys that the current user owns in the parent table being referenced in this security wizard session...
            string seQuery = "@" + _tableName + "." + _pkeyColumnName + " IN (" + pkeyCollection + ") AND @" + _tableName + ".owned_by = " + _cooperatorID;
            DataSet dsOwnedPkeys = _sharedUtils.SearchWebService(seQuery, true, true, null, _pkeyColumnName.Replace("_id", ""), 0, 0);
            if (dsOwnedPkeys.Tables.Contains("SearchResult"))
            {
                foreach (DataRow dr in dsOwnedPkeys.Tables["SearchResult"].Rows)
                {
                    string displayMember = _sharedUtils.GetLookupDisplayMember(_tableName + "_lookup", dr["ID"].ToString(), "", dr["ID"].ToString());
                    if (!ux_listboxRowRestrictions.Items.Contains(displayMember)) ux_listboxRowRestrictions.Items.Add(displayMember);
                }
            }
        }
