                    ...
                    _ux_NavigatorTabControl.SelectedTab.Name != "ux_tabpageGroupListNavigatorNewTab")
                {
                    ...
                    if (grinData.Tables.Contains(dt.TableName) && grinData.Tables[dt.TableName].Rows.Contains(drv[dt.PrimaryKey[0].ColumnName]))
                    {
                        ...
                        foreach (DataColumn dc in rawDR.Table.Columns)
                        {
                            ...
                            {
                                //linkedNodes = tv.SelectedNode.Nodes.Find(":" + dt.Columns[dt.PrimaryKey[0].ColumnName].ToString().Replace("_", "") + "=" + drv[dt.PrimaryKey[0].ColumnName].ToString(), ux_checkboxIncludeSubFolders.Checked);
                            ...
                            {
                                //linkedNodes = tv.SelectedNode.Nodes.Find(":" + dt.Columns[colName].ExtendedProperties["foreign_key_table_field_name"].ToString().Replace("_", "") + "=" + fKey, ux_checkboxIncludeSubFolders.Checked);
                                linkedNodes = tv.SelectedNode.Nodes.Find(":" + dc.ColumnName.Replace("_", "") + "=" + rawDR[dc.ColumnName].ToString(), true);
                                foreach (TreeNode tn in linkedNodes)
                                {
                                    // Get the font used by the TreeView because the NodeFont property is set to the default null...
...
string rawText = (string)e.Data.GetData("System.String");
                ...
                else if (e.Data.GetDataPresent(DataFormats.FileDrop))
                {
                        ...
                        hti.RowIndex > -1)
                    {
                        ...
                        // If the user dragged image file on to a dataview that has accession_id as the primary key...
...
//        }
        ...
        public DataObject BuildDragAndDropDGVData(DataGridView dgv)
        {
            ...
            // Create a dictionary of the column display order index to the column name...
            System.Collections.Generic.Dictionary<int, string> columnDisplayOrder = new System.Collections.Generic.Dictionary<int, string>();
            foreach (DataGridViewColumn dgvCol in dgv.Columns)
            {
                columnDisplayOrder.Add(dgvCol.DisplayIndex, dgvCol.Name);
            }

            // Now iterate through the dictionary to build the ordered column header...
            for (int i = 0; i < columnDisplayOrder.Count; i++)
            {
                string columnName = columnDisplayOrder[i];
                if (dgv.Columns[columnName].Visible)
                {
                    columnHeader += dgv.Columns[columnName].HeaderText + "\t";
                    columnNames += dgv.Columns[columnName].Name + "\t";
                }
            }
...
//if ((System.Windows.Forms.Control.ModifierKeys & Keys.Shift) == Keys.Shift)
            ...
            // Create an array of the sorted visible column names...
