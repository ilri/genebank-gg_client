...
namespace ImageForm
{
    ...
    public partial class ImageForm : Form, IGRINGlobalDataForm
    {
...
//public ImageForm(BindingSource bindingSource, bool performLookups, LookupTables lookupTables, bool editMode)
        public ImageForm(BindingSource bindingSource, bool performLookups, SharedUtils sharedUtils, bool editMode)
        {
            InitializeComponent();
...
//_lookupTables = lookupTables;
        ...
        public string PreferredDataview
        {
            get
            {
                return "get_accession_inv_attach";
            }
        }
            ...
            foreach (Control ctrl in controlCollection)
            {
                if (ctrl != ux_bindingnavigatorForm)  // Leave the bindingnavigator alone
                {
                        ...
                        ((DataTable)_bindingSource.DataSource).Columns.Contains(ctrl.Tag.ToString().Trim().ToLower()))
                    {
                        ...
                        ctrl.TabStop = !((DataTable)_bindingSource.DataSource).Columns[ctrl.Tag.ToString().Trim().ToLower()].ReadOnly;
        ...
        void control_TextChanged(object sender, EventArgs e)
        {
                ...
                ((DataTable)_bindingSource.DataSource).Columns.Contains(ctrl.Tag.ToString().Trim().ToLower()))
            {
                if (((DataTable)_bindingSource.DataSource).Columns[ctrl.Tag.ToString().Trim().ToLower()].ExtendedProperties.Contains("is_nullable") &&
                    ...
                    ((DataTable)_bindingSource.DataSource).Columns[ctrl.Tag.ToString().Trim().ToLower()].ExtendedProperties.Contains("is_readonly_on_insert") &&
                    ((DataTable)_bindingSource.DataSource).Columns[ctrl.Tag.ToString().Trim().ToLower()].ExtendedProperties["is_readonly_on_insert"].ToString().Trim().ToUpper() == "N" &&
        ...
        void textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != Convert.ToChar(Keys.Escape)) // Ignore the Escape key and process anything else...
            {
                ...
                if (!tb.ReadOnly)
                {
                    ...
                    LookupTablePicker ltp = new LookupTablePicker(_sharedUtils, tb.Tag.ToString(), ((DataRowView)_bindingSource.Current).Row, filterText);
