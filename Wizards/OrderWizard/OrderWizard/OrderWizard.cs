...
ggMessageBox.MessageText = string.Format(ggMessageBox.MessageText, argsArray);
        ...
        private void ux_datagridview_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //string thumbnailImageURL = ux_datagridviewOrderRequestAttach.Rows[e.RowIndex].Cells["thumbnail_virtual_path"].FormattedValue.ToString();
            //string fileName = System.IO.Path.GetFileName(thumbnailImageURL);
            //string tempFilesPath = System.IO.Path.GetTempPath();
//if (!thumbnailImageURL.Contains("~/uploads/imports/")) thumbnailImageURL = "~/uploads/imports/" + thumbnailImageURL; // HACK until PW puts full path into the database field 'thumbnail_virtual_path'
            //byte[] imageBytes = _sharedUtils.GetImage(thumbnailImageURL);
            //System.IO.Directory.CreateDirectory(tempFilesPath + "\\GRIN-Global CuratorTool");
            //System.IO.File.WriteAllBytes(tempFilesPath + "\\GRIN-Global CuratorTool\\" + fileName, imageBytes);
...
dr[dc.ColumnName + "_sortable"] = ((DateTime)dr[dc.ColumnName]).ToString("s");
                            }
                        }
                    }
                    else
...
//}
            ...
            {
                seQueryString = "@web_order_request.web_order_request_id IN (" + ux_textboxWebSelection.Text + ")";
            }
            ...
            {
                _webOrderRequestAction = ds.Tables["order_wizard_get_web_order_request_action"].Copy();
            }
            else
            {
            ...
            {
                _webOrderRequestAttach = ds.Tables["order_wizard_get_web_order_request_attach"].Copy();
            }
            else
            {
...
if (_orderRequestAction.Columns.Contains("completed_date")) newOrderRequestAction["completed_date"] = DateTime.Now;
                    ...
                    foreach (DataRow dr in orderRequestAttachChanges.Tables[_orderRequestAttach.TableName].Rows)
                    {
                            ...
                            (dr.RowState == DataRowState.Added || dr["virtual_path", DataRowVersion.Original] != dr["virtual_path", DataRowVersion.Current]))
                        {
                            ...
                            if (dr.Table.Columns.Contains("local_path") && !string.IsNullOrEmpty(dr["local_path"].ToString()))
                            {
                                ...
                                {
                                    returnedPath = _sharedUtils.SaveImage(virtualPath, attachBytes, true, true);
        ...
        void _orderRequestBindingSource_ListChanged(object sender, ListChangedEventArgs e)
        {
            ...
            formatControls(OrdersPage.Controls, _orderRequestBindingSource);
//ux_tabcontrolMain.SelectTab(OrdersPage);
        }
...
RefreshDGVFormatting(ux_datagridviewOrderRequestItem);
        ...
        private bool OrderItemsContainAlerts()
        {
            ...
            foreach (DataGridViewRow dgvr in ux_datagridviewOrderRequestItem.Rows)
            {
                ...
                {
                    if ((decimal)dgvr.Cells["quantity_on_hand"].Value - (decimal)dgvr.Cells["quantity_shipped"].Value < 1 &&
                        dgvr.Cells["quantity_on_hand_unit_code"].Value.ToString().Trim().ToUpper() == dgvr.Cells["quantity_shipped_unit_code"].Value.ToString().Trim().ToUpper())
                    {
                        hasAlerts = true;
                    }
                }
...
ux_bindingnavigatorForm.Visible = true;
                ...
                ux_groupboxWebOrderFilters.Visible = false;
                ux_groupboxPrinting.Visible = false;
                // Moving away from the Orders edit page so ensure no order is currently being left in edit mode...
...
RefreshDGVFormatting(ux_datagridviewOrderRequestItem);
        ...
        private DataRow BuildOrderRequestItemRow(string distributionInventoryID, DataTable destinationTable)
        {
            ...
            if (distributionInventory.Tables.Contains("order_wizard_get_inventory") &&
               distributionInventory.Tables["order_wizard_get_inventory"].Rows.Count > 0)
            {
                // Now populate the new order_request_item row with the inventory default distribution data...
                DataTable dt = distributionInventory.Tables["order_wizard_get_inventory"];
...
if (newOrderItem.Table.Columns.Contains("status_date")) newOrderItem["status_date"] = DateTime.Now.ToString("d");
                if (newOrderItem.Table.Columns.Contains("ipr_restriction") && dt.Columns.Contains("ipr_restriction") && inventoryDataRow["ipr_restriction"] != DBNull.Value) newOrderItem["ipr_restriction"] = inventoryDataRow["ipr_restriction"].ToString();
                if (newOrderItem.Table.Columns.Contains("xpvp_warning") && dt.Columns.Contains("xpvp_warning") && inventoryDataRow["xpvp_warning"] != DBNull.Value) newOrderItem["xpvp_warning"] = inventoryDataRow["xpvp_warning"].ToString();
                if (newOrderItem.Table.Columns.Contains("quarantine_restriction") && dt.Columns.Contains("quarantine_restriction") && inventoryDataRow["quarantine_restriction"] != DBNull.Value) newOrderItem["quarantine_restriction"] = inventoryDataRow["quarantine_restriction"].ToString();
                if (newOrderItem.Table.Columns.Contains("noxious_restriction") && dt.Columns.Contains("noxious_restriction") && inventoryDataRow["noxious_restriction"] != DBNull.Value) newOrderItem["noxious_restriction"] = inventoryDataRow["noxious_restriction"].ToString();
...
if (_orderRequest.Columns.Contains("completed_date")) currentOrderRequest["completed_date"] = DateTime.Now.ToString("d");
...
if (dr.Table.Columns.Contains("status_date_sortable")) dr["status_date"] = DateTime.Now.ToString("d");
                    }
...
orderRequest["completed_date"] = DateTime.Now.ToString("d");
                        }
                        else
                        {
...
//ApplyOrderItemBusinessRules(dgvr);
        ...
        private void RefreshDGVRowFormatting(DataGridViewRow dgvr)
        {
                ...
                dr.Table.Columns.Contains("quantity_shipped_unit_code"))
            {
                ...
                {
                    if (dr.GetColumnError("quantity_shipped") == "Shipped amount exceeds on-hand amount") dr.SetColumnError("quantity_shipped", null);
                }
            }
...
//RefreshOrderData();
                ...
                if (ux_tabcontrolMain.SelectedTab == WebOrderPage)
                {
                    if (ux_radiobuttonSelectionWebOrders.Checked)
                    {
                        ...
                        ux_textboxWebOrderDate.Enabled = false;
                        ux_textboxWebOrderDate.Visible = false;
                        ux_labelWebOrderDate.Visible = false;
                    ...
                    else
                    {
                        ...
                        ux_textboxWebOrderDate.Enabled = true;
                        ux_textboxWebOrderDate.Visible = true;
                        ux_labelWebOrderDate.Visible = true;
...
//if (newOrderRequest.Table.Columns.Contains("order_type_code")) newOrderRequest["order_type_code"] = "DI";
                ...
                webOrderRequest.Table.Columns.Contains("web_cooperator_id"))
            {
                ...
                string web_cooperator_id = webOrderRequest["web_cooperator_id"].ToString();
                string cooperator_id = FindCooperator(last_name, first_name, organization, geography_id, address_line1, web_cooperator_id);
...
CreateCooperator();
Cursor.Current = Cursors.WaitCursor;
                        cooperator_id = FindCooperator(last_name, first_name, organization, geography_id, address_line1, web_cooperator_id);
                        if (!string.IsNullOrEmpty(cooperator_id))
                        {
                            if (newOrderRequest.Table.Columns.Contains("final_recipient_cooperator_id")) newOrderRequest["final_recipient_cooperator_id"] = cooperator_id;
                        }
...
if (_orderRequestAction.Columns.Contains("completed_date")) newOrderRequestAction["completed_date"] = DateTime.Now;
            ...
            // Harvest any web order actions related to SMTA...
...
if (_orderRequestAction.Columns.Contains("completed_date")) newOrderRequestAction["completed_date"] = DateTime.Now;
                if (_orderRequestAction.Columns.Contains("completed_date_code")) newOrderRequestAction["completed_date_code"] = "MM/dd/yyyy";
                if (_orderRequestAction.Columns.Contains("action_information") && _webOrderRequestAction.Columns.Contains("action_for_id")) newOrderRequestAction["action_information"] = drv["action_for_id"];
                if (_orderRequestAction.Columns.Contains("action_cost")) newOrderRequestAction["action_cost"] = DBNull.Value;
                if (_orderRequestAction.Columns.Contains("cooperator_id")) newOrderRequestAction["cooperator_id"] = DBNull.Value;
                if (_orderRequestAction.Columns.Contains("note") && _webOrderRequestAction.Columns.Contains("note")) newOrderRequestAction["note"] = "Automatically transferred SMTAACCEPTED action from web_order_request_action table.  WEB NOTE: " + drv["note"];
                _orderRequestAction.Rows.Add(newOrderRequestAction);
            }
...
RefreshDGVFormatting(ux_datagridviewOrderRequestItem);
        ...
        private string FindCooperator(string last_name, string first_name, string organization, string geography_id, string address_line1, string web_cooperator_id)
        {
            ...
            //if (!string.IsNullOrEmpty(last_name)) findText += "@cooperator.last_name LIKE '" + last_name + "%'";
            ...
            // If a matching cooperator record was not found based on last_name, first_name, organization, geography_id, and address_line1 
            // then see if you can find a record with matching web_cooperator_ids (giving preference to the newest cooperator id record)...
            if (string.IsNullOrEmpty(cooperator_id))
            {
                if (cooperators.Columns.Contains("web_cooperator_id"))
                {
                    foreach (DataRowView drv in cooperators.DefaultView)
                    {
                        if (drv["web_cooperator_id"].ToString().Trim() == web_cooperator_id)
                        {
                            cooperator_id = drv["cooperator_id"].ToString().Trim();
                        }
                    }
...
string[] argsArray = new string[100];
argsArray[0] = saveCooperatorResults.Tables["ExceptionTable"].Rows[0]["Message"].ToString();
ggMessageBox.MessageText = string.Format(ggMessageBox.MessageText, argsArray);
                        ggMessageBox.ShowDialog();
                    }
                    else
...
//string[] virtualPathTokens = dr["virtual_path"].ToString().Split(new char[] { '/', '\\' });
//if (_orderRequestAttach.Select("virtual_path LIKE '%" + virtualPathTokens[virtualPathTokens.Length - 1] + "'").Length == 0)
...
//if (_orderRequestAttach.Columns.Contains("local_path")) drv["local_path"] = attachLocalPath;
                        ...
                        string returnedPath = "";
//// Get the byte array for the attachment...
//byte[] attachBytes = System.IO.File.ReadAllBytes(attachLocalPath);
//// Attempt to upload the image to the remote server...
//if (attachBytes != null && !string.IsNullOrEmpty(attachVirtualPath))
                        {
//returnedPath = _sharedUtils.SaveImage(attachVirtualPath, attachBytes, true, true);
//if (!string.IsNullOrEmpty(returnedPath))
                            {
                                DataRow newOrderRequestAttach = _orderRequestAttach.NewRow();
