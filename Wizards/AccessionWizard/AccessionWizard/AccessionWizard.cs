            ...
            foreach (Control ctrl in controlCollection)
            {
                if (ctrl != ux_bindingnavigatorForm)  // Leave the bindingnavigator alone
                {
                        ...
                        ((DataTable)_accessionBindingSource.DataSource).Columns.Contains(ctrl.Tag.ToString().Trim().ToLower()))
                    {
                        ...
                        ctrl.TabStop = !((DataTable)_accessionBindingSource.DataSource).Columns[ctrl.Tag.ToString().Trim().ToLower()].ReadOnly;
...
_accessionPKeys = ":accessionid=";
foreach (DataRow dr in _accession.Rows)
{
    _accessionPKeys += dr["accession_id"].ToString() + ",";
}
_accessionPKeys = _accessionPKeys.TrimEnd(',');
AccessionWizard_Load(sender, e);
            }
            else
            {
...
//if (ux_datagridviewAccessionNames.Columns.Contains("INVENTORY_ID")) ux_datagridviewAccessionNames.Columns["INVENTORY_ID"].DisplayIndex = i++;
        ...
        private void BuildAccessionSourcePage()
        {
            ...
            if (ds.Tables.Contains("get_accession_source"))
            {
                ...
                if (ux_datagridviewAccessionSource.Columns.Contains("IS_WEB_VISIBLE")) ux_datagridviewAccessionSource.Columns["IS_WEB_VISIBLE"].DisplayIndex = i++;
